<template>
  ...
  >
    ...
    <Transition name="fade-up">
      <div v-if="hovered" class="card-detail">
        ...
        <div class="detail-meta">
          ...
          <span v-if="card.fcStatus && card.fcStatus !== 'none'" class="detail-tag fc-tag">{{ fcLabel(card.fcStatus) }}</span>
        </div>
...
function fcLabel(fc: string): string {
  const m: Record<string, string> = { ap: 'AP', fcp: 'FC+', fc: 'FC' };
...
.badge-diff { opacity: 0.9; }
.badge-const { opacity: 0.75; font-weight: 500; font-size: 9px; }
